#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
#           This file was automatically generated from src/transformers/src/transformers/models/phi4_mini_ple/modular_phi4_mini_ple.py.
#               Do NOT edit this file manually as any edits will be overwritten by the generation of
#             the file from the modular. If any change should be done, please apply the change to the
#                          modular_phi4_mini_ple.py file directly. One of our CI enforces this.
#                🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨🚨
# coding=utf-8
...
from .configuration_phi4_mini_ple import Phi4MiniPleConfig


class Phi4MiniPleExpertMLP(nn.Module):
    """单个 MLP 专家（与 Phi3MLP 结构完全一致），用于 PLE 的双专家容器。
...
class Phi4MiniPleMLP(nn.Module):
    ...
    def __init__(self, config: Phi4MiniPleConfig):
        super().__init__()
        self.experts = nn.ModuleList([Phi4MiniPleExpertMLP(config) for _ in range(2)])
...
class Phi4MiniPleAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: Phi4MiniPleConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class Phi4MiniPleRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        Phi4MiniPleRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class Phi4MiniPleDecoderLayer(GradientCheckpointingLayer):
    """Decoder 层：Attention 与 Phi3 一致，MLP 替换为 Phi4MiniPleMLP，forward 透传 routing_index。
    ...
    def __init__(self, config: Phi4MiniPleConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = Phi4MiniPleAttention(config=config, layer_idx=layer_idx)
        # 将父类创建的 Phi3MLP 替换为 PLE 双专家容器
        self.mlp = Phi4MiniPleMLP(config)
        self.input_layernorm = Phi4MiniPleRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Phi4MiniPleRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.config = config
...
@auto_docstring
class Phi4MiniPlePreTrainedModel(PreTrainedModel):
    config: Phi4MiniPleConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["Phi4MiniPleDecoderLayer"]
    ...
    _can_record_outputs = {
        "hidden_states": Phi4MiniPleDecoderLayer,
        "attentions": Phi4MiniPleAttention,
    }
    _version = "0.0.5"


class Phi4MiniPleRotaryEmbedding(nn.Module):
    def __init__(self, config: Phi4MiniPleConfig, device=None):
        super().__init__()
...
def _determine_routing_index(input_ids: torch.LongTensor, config: Phi4MiniPleConfig) -> int:
...
@auto_docstring
class Phi4MiniPleModel(Phi4MiniPlePreTrainedModel):
    ...
    与 Phi3Model 的区别：
    - layers 使用 Phi4MiniPleDecoderLayer（双专家 MLP）
    ...
    def __init__(self, config: Phi4MiniPleConfig):
        ...
        # 将父类创建的 Phi3DecoderLayer 替换为 Phi4MiniPleDecoderLayer
        self.layers = nn.ModuleList(
            [Phi4MiniPleDecoderLayer(config, layer_idx=i) for i in range(config.num_hidden_layers)]
        )
        self.norm = Phi4MiniPleRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = Phi4MiniPleRotaryEmbedding(config=config)
...
@auto_docstring
class Phi4MiniPleForCausalLM(Phi4MiniPlePreTrainedModel, GenerationMixin):
    """Causal LM 顶层：使用 Phi4MiniPleModel 作为 backbone。
    ...
    def __init__(self, config: Phi4MiniPleConfig):
        super().__init__(config)
        self.model = Phi4MiniPleModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ```python
        >>> from transformers import AutoTokenizer, Phi4MiniPleForCausalLM

        >>> model = Phi4MiniPleForCausalLM.from_pretrained("meta-phi4_mini_ple/Phi4MiniPle-2-7b-hf")
        >>> tokenizer = AutoTokenizer.from_pretrained("meta-phi4_mini_ple/Phi4MiniPle-2-7b-hf")
...
__all__ = [
    "Phi4MiniPleExpertMLP",
    "Phi4MiniPleMLP",
    "Phi4MiniPleDecoderLayer",
    "Phi4MiniPleModel",
    "Phi4MiniPleForCausalLM",
]
